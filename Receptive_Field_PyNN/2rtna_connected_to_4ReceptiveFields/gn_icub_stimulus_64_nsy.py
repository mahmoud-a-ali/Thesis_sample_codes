...
Y     = Yi+ Yc+ Yu +Yb
#add noise
for i in range(1, len(X_lft), 7 ):
    X_lft[i] = random.randint(0,rtna_w)
    Y[i]=random.randint(0,rtna_h)
...
#add noise
for i in range(1, len(X_rght), 7 ):
    X_rght[i] = random.randint(0,rtna_w)
    Y[i]=random.randint(0,rtna_h)
